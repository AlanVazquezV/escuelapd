...
class Components_dataController extends Controller{
	...
	/**
     * classes_r_option_list Model Action
     * @return array
     */
	function classes_r_option_list(Request $request){
		$sqltext = "SELECT  DISTINCT id AS value,classes AS label FROM classes_r ORDER BY id ASC";
		$query_params = [];
	...
	/**
     * check if name value already exist in User
	 * @param string $value
     * @return bool
     */
	function user_name_exist(Request $request, $value){
		$exist = DB::table('user')->where('name', $value)->value('name');   
		if($exist){
			return "true";
		}
		return "false";
