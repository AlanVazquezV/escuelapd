...
class UserController extends Controller
{
	...
	function index(Request $request, $fieldname = null , $fieldvalue = null){
		...
		$query->join("cycle", "user.cycle", "=", "cycle.id");
     ...
     * @return \Illuminate\View\View
     */
	function alumnos_lista(Request $request, $fieldname = null , $fieldvalue = null){
		$query = User::query();
		if($request->search){
			$search = trim($request->search);
			User::search($query, $search);
		...
		$query->join("schedule_name", "user.schedule", "=", "schedule_name.id");
		$query->join("schedule", "schedule_name.id", "=", "schedule.name");
		$query->join("classes", "schedule.classes", "=", "classes.id");
		$orderby = $request->orderby ?? "user.id";
		$ordertype = $request->ordertype ?? "desc";
		$query->orderBy($orderby, $ordertype);
		$query->where("type", "=" , 1);
		...
		$records = $this->paginate($query, User::alumnosListaFields());
		return $this->respond($records);
	}
}
