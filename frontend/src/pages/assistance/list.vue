                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/assistance/add')">
                                <router-link :to="`/assistance/add`">
                                    <Button label="Agregar asistencia" icon="pi pi-plus" type="button" class="p-button bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                        ...
                        <div >
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"     :value="records" dataKey="id" @sort="onSort($event)" class="" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    ...
                                    <Column  field="classes_r_date" header="Fecha" >
                                        <template #body="{data}">
                                            {{ data.classes_r_date }}
                                        </template>
                                    </Column>
                                    <Column  field="user_name" header="Nombre" >
                                        <template #body="{data}">
                                            {{ data.user_name }}
                                        </template>
                                    </Column>
                                    <Column  field="user_lastname" header="Apellido" >
                                        <template #body="{data}">
                                            {{ data.user_lastname }}
                                        </template>
                                    </Column>
                                    <Column  field="user_username" header="Usuario" >
                                        <template #body="{data}">
                                            {{ data.user_username }}
                                        </template>
                                    </Column>
                                    <Column  field="assistance_confirmation_label" header="Asistencia" >
                                        <template #body="{data}">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: "Confirmar",
			command: (event) => { app.openPageDialog({ page:'assistance/confirmar', url: `/assistance/confirmar/${data.id}`, closeBtn: true }) },
			icon: "pi pi-check",
			visible: auth.canView('assistance/confirmar')
		}
	]
	}
