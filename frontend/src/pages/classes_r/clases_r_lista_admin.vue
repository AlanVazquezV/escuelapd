                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/classes_r/add')">
                                <router-link :to="`/classes_r/add`">
                                    <Button label="Add New Classes R" icon="pi pi-plus" type="button" class="p-button bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/classes_r'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{data}">
                                                <router-link :to="`/classes_r/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('classes_r/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		pageStoreKey: {
			type: String,
			default: 'CLASSES_R-CLASES_R_LISTA_ADMIN',
		},
		pageName: {
			type: String,
			default : 'classes_r',
		...
		routeName: {
			type: String,
			default: 'classes_rclases_r_lista_admin',
		},
		apiPath: {
			type: String,
			default: 'classes_r/clases_r_lista_admin',
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/classes_r/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('classes_r/view')
		},
		{
			label: "Edit",
			to: `/classes_r/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('classes_r/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('classes_r/delete')
		},
		{
			label: "Asistencia",
			to: `/assistance/asiss_lista/${data.id}`,
			icon: "pi pi-align-right",
			visible: auth.canView('assistance/asiss_lista')
		},
		{
			label: "Reservar",
			to: `/assistance/asiss_reserv/${data.id}`,
			icon: "pi pi-angle-double-up",
			visible: auth.canView('assistance/asiss_reserv')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Classes R";
